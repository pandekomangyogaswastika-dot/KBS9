...
from demos.kn3.core_utils import now_iso, new_id
from demos.kn3.schemas import WAREHOUSE_PRIORITY
...
async def expire_old_reservations() -> int:
    """Expire reservations older than 3 days."""
    ...
    for order in orders:
        for allocation in order.get("allocations", []):
            await db.inventory_balances.update_one(
                {"product_id": allocation["product_id"], "warehouse_id": allocation["warehouse_id"]},
                {
                    "$inc": {"reserved_qty": -allocation["quantity"], "available_qty": allocation["quantity"]},
                    "$set": {"updated_at": now_iso()},
                },
            )
        ...
        from demos.kn3.dependencies import audit
        await audit(
...
async def render_order_html(
    ...
    customer = safe_doc(await db.customers.find_one({"id": order["customer_id"]}, {"_id": 0}))
    ...
    rows = "".join(
        f"<tr><td>{item['sku']}</td><td>{item['product_name']}</td><td>{item['quantity']}</td>"
        f"<td>{item['unit']}</td><td>Rp {item['price']:,.0f}</td><td>Rp {item['subtotal']:,.0f}</td></tr>"
        for item in order["items"]
    )
    allocation_rows = "".join(
        f"<li>{a['warehouse_name']} ({a['warehouse_city']}) — {a['quantity']} "
        f"{next((i['unit'] for i in order['items'] if i['product_id']==a['product_id']), '')}</li>"
    ...
    sections = {
        "header": (
            ...
            f"<p>{order['number']}</p><p>{datetime.now(timezone.utc).strftime('%d %b %Y')}</p></div></div>"
        ),
        "customer": (
            f"<section><h3>Customer & Tujuan</h3><p><b>{customer['name']}</b> — {customer['pic_name']}<br/>"
        ...
        "items": (
            ...
            f"<h2>Total: Rp {order['total_amount']:,.0f}</h2></section>"
        ),
        ...
        "signature": (
            ...
            f"<br/><br/><b>{address.get('recipient_name', customer['pic_name'])}</b></div></div>"
        ),
