...
from demos.kn3.schemas import ScannerScan, WMSTaskCreate
...
async def create_outbound_from_order(order_id: str, request: Request) -> List[Dict[str, Any]]:
    ...
    for alloc in order.get("allocations", []):
        ...
        item = next((i for i in order["items"] if i["product_id"] == alloc["product_id"]), {})
        stages = FLOW_STAGES["outbound"]
        task = {
            ...
            "sku": product.get("sku", ""), "quantity": alloc["quantity"],
            "unit": item.get("unit", "unit"), "warehouse_id": alloc["warehouse_id"],
            "warehouse_name": warehouse.get("name", ""), "warehouse_city": warehouse.get("city", ""),
        ...
        await db.wms_tasks.insert_one(task)
        created_tasks.append(safe_doc(task))
...
async def advance_task(task_id: str, request: Request) -> Dict[str, Any]:
    ...
    if next_stage in ["done", "dispatched"] and task["flow_type"] == "outbound":
        await db.inventory_balances.update_one(
            {"product_id": task["product_id"], "warehouse_id": task["warehouse_id"]},
            {"$inc": {"on_hand_qty": -task["quantity"], "reserved_qty": -task["quantity"]},
             "$set": {"updated_at": now_iso()}}
        )
        await db.inventory_movements.insert_one({
            "id": new_id("mov"), "product_id": task["product_id"],
            "warehouse_id": task["warehouse_id"], "movement_type": "outbound",
            "quantity": -task["quantity"], "unit": task["unit"],
            "batch": task["batch"], "lot": task["lot"], "roll_id": task["roll_id"],
            "source_document": task_id, "timestamp": now_iso()
        })
    updated = await db.wms_tasks.find_one_and_update(
