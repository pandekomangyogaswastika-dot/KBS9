"""AI Report Generation Service for Assessment Results - Production Version.

PRODUCTION DEPLOYMENT: Uses direct Anthropic SDK instead of emergentintegrations.
emergentintegrations library only available in Emergent environment.

For VPS deployment: Use this file and ANTHROPIC_API_KEY environment variable.
"""
import os
import asyncio
...
# Use ANTHROPIC_API_KEY for VPS deployment (standard Anthropic SDK)
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY")


class AIReportGenerator:
    """Generate AI-powered insights for completed assessments using direct Anthropic SDK."""
    ...
    async def _analyze_domain(self, domain: Dict, answers_map: Dict, locale: str) -> Optional[Dict]:
        ...
        try:
            ...
            parsed = self._parse_domain_response(response_text, domain_title)
            return parsed
...
try:
    ...
    print(f"⚠️  Warning: Failed to initialize AIReportGenerator: {str(e)}")
