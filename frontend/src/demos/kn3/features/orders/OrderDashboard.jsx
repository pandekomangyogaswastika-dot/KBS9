...
function OrderDashboard({ orders = [] }) {
  ...
  const metrics = useMemo(() => {
    ...
    const totalRevenue = recentOrders
      .filter(o => ["confirmed", "dispatched", "done"].includes(o.status))
          ...
          <div className="section-body">
            <div className="space-y-2">
              {[
                ...
                { label: "Confirmed", count: metrics.statusCounts.confirmed, color: "bg-[#34C759]" },
                { label: "Dispatched", count: metrics.statusCounts.dispatched, color: "bg-[#5856D6]" },
              ...
              <tbody className="divide-y divide-[#EFF0F2]">
                {metrics.recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-[#FAFBFC]">
                    ...
                    <td className="px-3 py-2 text-[11.5px] font-semibold text-right">{formatCurrency(order.total_amount)}</td>
