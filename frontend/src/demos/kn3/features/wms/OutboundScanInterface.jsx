...
export default function OutboundScanInterface({ user }) {
  ...
  const handleDispatch = async () => {
    ...
    if ((selectedTask.picked_qty || 0) < selectedTask.quantity)
      return alert("Qty belum sesuai! Escalate ke manager jika tidak bisa memenuhi.");
    if (!window.confirm("Yakin ingin dispatch?")) return;
    setSubmitting(true);
    try {
      await axios.post(`/api/outbound/tasks/${selectedTask.id}/dispatch`);
      fetchTasks();
  ...
  const FILTERS = ["all", "created", "picking", "packing", "staging", "escalated"];
  const FILTER_LABELS = { all: "Semua", created: "Created", picking: "Picking", packing: "Packing", staging: "Staging", escalated: "Escalated" };
              ...
              {selectedTask.status === 'escalated' && selectedTask.escalation && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-2 text-[11px]">
                  ...
                  {selectedTask.escalation.status === 'resolved' && (
                    <p className="text-green-700 mt-1">✓ Resolved: {selectedTask.escalation.resolution_notes}</p>
                  )}
                </div>
              )}
              ...
              {!['dispatched', 'escalated'].includes(selectedTask.status) && (
                <>
                  <div className="flex items-center gap-2">
                    ...
                    {scanValue && <span className="text-[11px] text-green-700 font-medium">✓ {scanValue}</span>}
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    {(selectedTask.picked_qty || 0) >= selectedTask.quantity && (
              ...
              {selectedTask.status === 'dispatched' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[12px] font-semibold text-green-700">Dispatched!</p>
                      <p className="text-[11px] text-green-600">{formatQty(selectedTask.picked_qty)} {selectedTask.unit} terkirim</p>
                    </div>
