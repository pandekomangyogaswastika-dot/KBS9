/**
 * SerialTrackingPanel.jsx
 *
 * Adapted dari `SerialTrackingModule.jsx` di repo `garmentyanathisfinal`,
 * disesuaikan agar:
 *  - Tidak butuh token (public demo endpoints)
 *  - Memakai `REACT_APP_BACKEND_URL` via services/api.js
 *  - Punya data-testid yang konsisten dengan standar KBS
 */
...
import { fetchSerialList, fetchSerialTrace } from "../services/api";

const STEP_ICONS = {
  "PO Created": Package,
  "Vendor Shipment": Truck,
  "Material Inspection": ClipboardCheck,
  "Material Request": Package,
  "Production Job": Factory,
  "Production Progress": BarChart3,
  "Buyer Dispatch": ShoppingBag,
  "Production Return": RotateCcw,
};
const STEP_COLORS = {
  "PO Created": "bg-blue-500",
  "Vendor Shipment": "bg-purple-500",
  "Material Inspection": "bg-emerald-500",
  "Material Request": "bg-amber-500",
  "Production Job": "bg-indigo-500",
  "Production Progress": "bg-cyan-500",
  "Production Return": "bg-rose-500",
  "Buyer Dispatch": "bg-purple-600",
};
...
export default function SerialTrackingPanel() {
  ...
  const loadList = useCallback(async () => {
    setListLoading(true);
    try {
      const items = await fetchSerialList({ status: listFilter, search: listSearch });
      setSerialList(items);
    } catch (e) {
  ...
  const loadTraceForSerial = async (sn) => {
    ...
    try {
      const t = await fetchSerialTrace(sn);
      setTraceData(t);
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error(e);
  ...
  useEffect(() => {
    ...
    if (searchInput.trim().length >= 2) {
      debounceRef.current = setTimeout(() => fetchTrace(searchInput), 400);
    } else {
      setData(null);
    }
  ...
  return (
    <div className="space-y-5" data-testid="garment-serial-panel">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Serial Tracking</h2>
          <p className="text-slate-500 text-sm mt-1">
            Lacak dan monitor seluruh nomor serial pada alur produksi garmen
          </p>
        </div>
      ...
      {tab === "list" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <button
              onClick={() => setListFilter("ongoing")}
              className={`rounded-xl p-4 border text-left transition-all ${
                listFilter === "ongoing"
                  ? "border-blue-300 bg-blue-50 shadow-md"
                  : "border-slate-200 bg-white hover:border-blue-200"
              }`}
              data-testid="summary-ongoing"
            >
              ...
              <div className="w-full bg-blue-100 rounded-full h-1.5 mt-2">
                <div
                  ...
                  style={{
                    width: `${
                      serialList.length ? (ongoingCount / serialList.length) * 100 : 0
                    }%`,
                  }}
                ></div>
              </div>
            ...
            >
              ...
              <div className="w-full bg-emerald-100 rounded-full h-1.5 mt-2">
                <div
                  ...
                  style={{
                    width: `${
                      serialList.length ? (completedCount / serialList.length) * 100 : 0
                    }%`,
                  }}
                ></div>
              </div>
            ...
            >
              ...
              <div className="w-full bg-amber-100 rounded-full h-1.5 mt-2">
                <div
                  ...
                  style={{
                    width: `${
                      serialList.length ? (pendingCount / serialList.length) * 100 : 0
                    }%`,
                  }}
                ></div>
              </div>
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Cari nomor serial, produk, PO, buyer..."
                ...
                data-testid="serial-list-search"
              />
            </div>
            <button
          ...
          {listLoading ? (
            ...
            <div className="text-center py-16 text-slate-400" data-testid="empty-state">
              ...
              <p className="text-sm">Tidak ada nomor serial ditemukan</p>
            </div>
          ) : (
            <div className="space-y-2">
              {serialList.map((s, idx) => (
                <div
                  key={`${s.serial_number}-${s.po_id}-${idx}`}
                  className="bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-blue-200 transition-all"
                  data-testid={`serial-card-${s.serial_number}`}
                >
                  <button
                    onClick={() => loadTraceForSerial(s.serial_number)}
                    className="w-full text-left p-4 flex items-center gap-4"
                    data-testid={`serial-row-${idx}`}
                  >
                    <div
                    ...
                    <div className="hidden md:grid grid-cols-4 gap-4 text-center flex-shrink-0">
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-700">
                          {formatNum(s.ordered_qty)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400">Diterima</p>
                        <p className="text-sm font-bold text-emerald-600">
                          {formatNum(s.received_qty)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400">Produksi</p>
                        <p className="text-sm font-bold text-blue-600">
                          {formatNum(s.produced_qty)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400">Kirim</p>
                        <p className="text-sm font-bold text-purple-600">
                          {formatNum(s.shipped_qty)}
                        </p>
                      </div>
                    </div>
                    <div className="flex-shrink-0">
                      {expandedSerial === s.serial_number ? (
                    ...
                    <div className="border-t border-slate-100 bg-slate-50 p-4">
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-4">
                        <div className="bg-white rounded-lg p-2.5 border border-slate-100 text-center">
                          ...
                          <p className="text-sm font-bold text-slate-700">{s.po_number}</p>
                        </div>
                        <div className="bg-white rounded-lg p-2.5 border border-slate-100 text-center">
                        ...
                        <div className="bg-white rounded-lg p-2.5 border border-slate-100 text-center">
                          ...
                          <p className="text-sm font-bold text-blue-600">{s.po_status}</p>
                        </div>
                        <div className="bg-white rounded-lg p-2.5 border border-slate-100 text-center">
                          <p className="text-xs text-slate-400">Remaining</p>
                          <p className="text-sm font-bold text-amber-600">
                            {formatNum(s.remaining_qty)} pcs
                          </p>
                        </div>
                      </div>
                      {(traceData.timeline || []).length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-slate-600 mb-2">
                            Timeline ({traceData.timeline.length} events)
                          </p>
                                ...
                                <div key={i} className="flex items-center gap-2.5">
                                  ...
                                  <div className="flex-1 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      {ev.received_qty > 0 && (
                                        <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1 rounded">
                                          +{ev.received_qty}
                                        </span>
                                      )}
                      ...
                      <button
                        onClick={() => {
                          setTab("trace");
                          setSearchInput(s.serial_number);
                          fetchTrace(s.serial_number);
                        }}
                        className="mt-3 text-xs text-blue-600 hover:text-blue-800 font-medium"
                        data-testid={`go-trace-${s.serial_number}`}
                      >
                        Lihat Trace Lengkap →
                      </button>
                    </div>
      ...
      {tab === "trace" && (
        <div className="space-y-5">
          <form onSubmit={handleSearch} className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Ketik nomor serial (contoh: SN-2026-0001) — otomatis mencari..."
          ...
          {data && data.serial_number && data.po_item_count > 0 && (
            <div className="space-y-5">
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      {data.po_count} PO | {data.po_item_count} item ditemukan |{" "}
                      ...
                      {allItems.map((item, idx) => (
                        <tr
                          ...
                          className={`border-b border-slate-50 ${
                            item.is_searched_serial ? "bg-blue-50" : "hover:bg-slate-50"
                          }`}
                        >
                          <td className="px-4 py-2 font-medium">
                            {item.serial_number}{" "}
                            {item.is_searched_serial && (
                      ...
                      {data.timeline.map((event, idx) => {
                        ...
                        return (
                          <div key={idx} className="relative flex gap-4 pl-2">
                            <div
                              className={`relative z-10 w-9 h-9 ${color} rounded-full flex items-center justify-center flex-shrink-0 shadow-sm`}
                            >
                              <Icon className="w-4 h-4 text-white" />
                            </div>
                            <div
                              className="flex-1 bg-white rounded-xl border border-slate-200 p-3 hover:border-slate-300 transition cursor-pointer"
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-1.5">
                                ...
                                {event.qty_sent > 0 && (
                                  <span className="text-xs bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded">
                                    Dikirim: {event.qty_sent}
                                  </span>
                                )}
                                {event.received_qty !== undefined && (
                                  <span className="text-xs bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded">
                                    Diterima: {event.received_qty}
                                  </span>
                                )}
                                {event.missing_qty > 0 && (
                                  <span className="text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded">
                                    Missing: {event.missing_qty}
                                  </span>
                                )}
                                {event.produced_qty !== undefined &&
                                  event.produced_qty > 0 && (
                                    <span className="text-xs bg-cyan-100 text-cyan-600 px-1.5 py-0.5 rounded">
                                      Produksi: {event.produced_qty}
                                    </span>
                                  )}
                                {event.completed_quantity > 0 && (
                                  <span className="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">
                                    +{event.completed_quantity} pcs
                                  </span>
                                )}
                                {event.qty_shipped > 0 && (
                                  <span className="text-xs bg-amber-100 text-amber-600 px-1.5 py-0.5 rounded">
                                    Kirim: {event.qty_shipped}
                                  </span>
                                )}
                                {event.status && (
                                  <span className="text-xs bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">
                              ...
                              {isExpanded && (
                                <div className="mt-2 pt-2 border-t border-slate-100 grid grid-cols-2 gap-1.5">
                                  {Object.entries(event)
                                    .filter(([k]) => !["step", "date", "module"].includes(k))
                                    .map(
                                      ([k, v]) =>
                                        v !== null &&
                                        v !== undefined &&
          ...
          {data && data.po_item_count === 0 && !loading && (
            <div className="text-center py-16" data-testid="trace-not-found">
              <Search className="w-16 h-16 text-slate-200 mx-auto mb-4" />
              <p className="text-slate-500 text-lg">Serial number tidak ditemukan</p>
              <p className="text-slate-400 text-sm mt-1">
                Coba salah satu: SN-2026-0001 sampai SN-2026-0012
              </p>
            </div>
          )}
          ...
          {!data && !loading && tab === "trace" && (
            <div className="text-center py-16">
              ...
              <p className="text-slate-300 text-sm mt-1">
                Otomatis mencari saat mengetik (min. 2 karakter)
              </p>
            </div>
          )}
        </div>
